...
describe("computeScale", () => {
  ...
  it("snaps line-height to rhythm grid", () => {
    ...
    for (const item of result) {
      assert.equal(item.lineHeight % 4, 0, `${item.label} line-height ${item.lineHeight} not divisible by 4`);
    }
  });
  ...
  it("works with different rhythm values", () => {
    ...
    for (const item of result) {
      assert.equal(item.lineHeight % 8, 0, `${item.label} line-height ${item.lineHeight} not divisible by 8`);
    }
  });
});
